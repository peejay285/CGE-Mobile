...
class _PremiumUpgradeScreenState extends ConsumerState<PremiumUpgradeScreen> {
  ...
  Future<void> _pay(Profile profile) async {
    ...
    try {
      final reference = PaymentService.generateReference('premium');
      final ok = await PaymentService.pay(
        context: context,
        amountInKobo: PaymentService.toKobo(_premiumPriceNaira),
        email: user.email!,
        reference: reference,
        metadata: {
          'type': 'premium',
          'user_id': user.id,
          'period_days': _premiumPeriodDays,
        },
      );
      if (!mounted) return;
      if (ok) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text(
              'Payment received. Premium will activate once the webhook lands (a few seconds).',
            ),
            duration: Duration(seconds: 4),
          ),
        );
        // Give the webhook a moment, then refetch the profile.
                ...
                child: Column(
                  ...
                  children: [
                    Row(
                      ...
                      children: [
                        Text('₦${_premiumPriceNaira.toStringAsFixed(0)}',
                      ...
                      child: Row(
                        ...
                        children: [
                          Container(
                            ...
                            child:
                                Icon(p.$1, size: 16, color: AppColors.cyan),
                          ),
                          const SizedBox(width: 10),
                          Expanded(
                            child: Column(
